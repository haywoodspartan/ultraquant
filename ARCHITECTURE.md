...
**Version 5.04 · 2155 tests green · pure-Python core with optional C++/CUDA acceleration**
...
smaller one. A transformer spends its entire parameter space on every
token because a dense forward pass has no way to do less; this is a
**catalogued, pageable shard library** that reads the shards a question
points at and leaves the rest on storage. Around it sit an interpreter
that reasons over facts it can cite and revise, and an uncertainty
layer that lets every tier say *"I don't know"* — the one thing a
softmax structurally cannot express.

Everything in the core is written from scratch against the Python
standard library: the qubit engine, the neural network, the memory, the
archive. Native acceleration and a real QPU are strictly *optional
accelerants* — the pure-Python tier defines the semantics and every
other tier is verified to reproduce it.

**The claims are gated, including the ones that failed.** Each
capability has a falsifiable measurement written before it ran, and
[§12](#12-how-this-differs-from-a-dense-transformer-measured) collects
what those measurements actually settled — where this architecture
beats a dense model, where it does not, and what remains open.
...
- [11. The distance to general](#11-the-distance-to-general) — the
  assessment, the two things missing, and six staged gates
  ([11.5](#115-stage-1-was-run-and-it-failed) failed,
  [11.6](#116-stage-2-was-run-and-it-passed),
  [11.7](#117-hypervectors-half-a-pass),
  [11.8](#118-stage-3-was-run-and-it-passed),
  [11.9](#119-stage-4-was-run-and-it-passed),
  [11.10](#1110-stage-5-was-run-and-it-passed) passed)
- [12. How this differs from a dense transformer, measured](#12-how-this-differs-from-a-dense-transformer-measured)
- **[JOURNAL.md](JOURNAL.md)** — the 94 units built since, each with its
...
### 11.11 onward — the development journal

Ninety-four units of work follow the staged path above, and they live
in **[JOURNAL.md](JOURNAL.md)** with their own index. Each is one
capability, the gate written before it ran, and the verdict written
after.

They were split out when the journal reached 4,198 lines against this
document's 2,745, and only seven of its ninety-four sections appeared
in any table of contents. A record nobody can navigate is a record
nobody reads.

What the journal contains that this document does not: the native
C++ cognitive tier and its parity gates, the arithmetic and question-
form work, the transformer conversion kit, the 27-block encoder, the
uncertainty layer, and every failure among them.

**The verdicts that changed the design are summarised in
[§12](#12-how-this-differs-from-a-dense-transformer-measured).**

---

## 12. How this differs from a dense transformer, measured

The claim behind this architecture is not that it is a better language
model. It is that a **dense forward pass is the wrong shape** for
several things, and that a catalogued library with an uncertainty layer
does them differently. That is a testable claim, and this section
collects what the tests said — including where they said no.

### 12.1 Where it is measurably different

**Selective activation.** A transformer spends 100% of its parameters
on every token; it has no catalog to consult. Measured on a
twelve-expert library ([§11.104](JOURNAL.md#11104-reading-until-you-know-enough-failed-kept)):

| experts read | 1 | 2 | **3** | 4 | 12 |
|---|---:|---:|---:|---:|---:|
| accuracy | 0.9450 | 0.9733 | **0.9900** | 0.9917 | 0.9900 |

**A quarter of the library buys all of the accuracy.** The resident set
is bounded by a byte budget rather than by the model's size, which is
...
The encoder result is the sharpest: a converted tower whose output had
been reduced to cosine 0.0185 stayed finite, correctly shaped, and
plausible all the way through. The non-finite check that production
code actually runs **never fired**. Attention entropy caught it.

**Provenance and editability.** A fact and a hallucination are the same
weights in a transformer, and correcting either means retraining. Here
a claim carries confidence, polarity, and a revision history; web
intake is quarantined as *found, not believed*
([§4.2](#42-web-intake--found-vs-believed)); a derived answer names
every premise and can be vetoed premise by premise; and a correction is
one write.

**Exact arithmetic.** Exact rationals, and proved decimal enclosures
for irrational roots rather than a decimal that looks exact and is not.
...
answer is that this is not a shortcut to a shared representation:
0.467 relative output error per layer, compounding over 27 blocks to
**cosine 0.0185** — the converted encoder's output is not degraded, it
is uncorrelated, and it never becomes selective.

**Entropy does not route.** The obvious extension — let uncertainty
decide how many shards to read — was built and lost to a plain constant
`k` three separate ways. Reads varied with sd 0.76, under a one-read
bar. Where uncertainty has earned its place is **refusal**, not
retrieval.

**Not competitive at open-domain language**, and nothing here claims
otherwise. Facts must be told rather than read at corpus scale, and the
